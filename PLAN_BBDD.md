...
- **2026-06-26** — **Paridad de UI: Nueva Obra/Detalle = Presupuesto BBDD.** El plan generado por el motor BBDD muestra en `PlanTable` (lectura): líneas a revisar en amarillo (nueva columna `plan_rows.needs_review`, migración v9), confianza por ensayo (Confidence reconoce `tarifa_cye`) y desplegable "¿Por qué este ensayo?" (procedencia + confianza). typecheck+build verdes.

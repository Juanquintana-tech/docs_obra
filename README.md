...
lo valora con tarifas reales de laboratorio (catálogo ALAGAL) y produce los entregables en Excel y
Word. Incluye además gestión de proyectos e **informes de ensayo de campo** (densidad in situ,
placa de carga) con cálculo automático y veredicto de cumplimiento.
...
MiniMax (clasificación IA) · exceljs / docxtemplater (entregables) · unpdf / mammoth (ingesta).
...
│   ├── ipc.ts                  # Handlers IPC (único punto que toca el backend)
│   ├── db/                     # SQLite (better-sqlite3) + migraciones por user_version
│   ├── services/pipeline.ts    # Orquesta ingesta, entregables y consultas RAG (cachea RAG/reglas)
│   └── pipeline/               # Núcleo, puro y testeable con tsx (sin Electron)
...
│       ├── rag/                # Valoración de precios (TF-IDF char n-gram; embeddings enchufables)
│       ├── ensayos.ts          # Motor de cálculo de informes de campo (densidad, placa)
│       ├── informes.ts         # Generación Word/Excel de informes de ensayo
│       ├── formatter/          # Entregables del plan: Excel (exceljs) + Word (plantilla docxtemplater)
...
└── renderer/src/               # UI React
    ...
    ├── pages/                  # Dashboard, Proyectos, NuevaObra, Detalle, Ensayos, Presupuestos, ValidacionRag
    ├── components/             # Sidebar, PlanTable (+ EditablePlanTable)
    └── lib/                    # api, tipos de dominio, formato

resources/
├── knowledge/                  # test_rules.json (reglas normativas) · tarifas_alagal.xlsx (catálogo)
└── templates/                  # plan_plantilla.docx (editable en Word, marcadores docxtemplater)
```
...
- **RAG de precios:** TF-IDF de n-gramas de caracteres (3-5) sobre el catálogo ALAGAL. Cada fila
  del plan lleva su **nivel de confianza** (`price_source` + `rag_score`), visible en la UI.
  La arquitectura admite embeddings (híbrido) sin reescribir — pendiente de activar.
- **Confianza e iteración:** la pantalla **Validación RAG** permite ojear y medir los matches.

## Modelo de datos (SQLite, migraciones por `user_version`)

- **`obras`** — proyecto (obra, cliente, ref_lab, fecha, responsable, totales, status).
- **`plan_rows`** — filas del plan (FK→obras, ON DELETE CASCADE), con `price_source`/`rag_score`/`rag_desc`.
- **`ensayos`** — informes de campo (tipo, estado, veredicto, `datos` JSON).
- **`price_corrections`** — correcciones de precio del usuario (auditoría + futuro aprendizaje del RAG).
...
Crea un `.env` en la raíz (ignorado por git) a partir de [`.env.example`](.env.example):

```
MINIMAX_API_KEY=tu_clave
```

El resto de la app (proyectos, ensayos, descargas) funciona sin clave; solo la clasificación de
materiales del documento la requiere.
...
| `npm run template:build`          | Regenera la plantilla Word base (`resources/templates/plan_plantilla.docx`) |
...
`npm run build:mac` produce un `.dmg` (en `dist/`, no versionado). Para distribución profesional
faltan: firma + notarización Apple (Developer ID), build de Windows vía CI, y opcionalmente
universal/arm64. Ver [`PLAN.md`](PLAN.md).
